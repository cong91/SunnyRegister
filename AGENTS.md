...
## Project Guide (generated 2026-09-05, refreshed 2026-09-14)
...
- `backend/`: Go 1.23 module for the HTTP API, persistence, task scheduling, auth, audit, and integrations. Backend code lives as a single flat package (e.g. `main.go`, `db.go`, `sunny_*.go`, `tasks.go`) with colocated `_test.go` files; `sunny_register.go` is already very large, so place new logic in a fitting sibling file rather than growing it.
- `python-worker/`: Python 3.12+ FastAPI worker (`worker.py`, `sunny_runner.py`) and isolated task runtime packages: `direct_card_runtime/`, `gopay_runtime/`, `paypal_runtime/`, `sunny_core/`, plus `tests/` and `tools/`.
- `frontend/`: React 19 + TypeScript + Vite 8 console with Tailwind CSS 4, Radix slot, GSAP, and Lucide. Source is under `frontend/src/` (`pages/`, `components/`, `lib/`, `App.tsx`); i18n assets are generated by `npm run i18n:generate` (Vietnamese map) and smoke-checked by `npm run test:i18n`.
...
- `cd frontend; npm ci; npm run lint; npm run build` (build runs `tsc -b && vite build`); after i18n changes also `npm run test:i18n`.
